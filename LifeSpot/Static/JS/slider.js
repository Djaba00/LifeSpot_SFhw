...
const SliderDragCursorClassName = 'slider-drag-cursor';
...
const SliderNavButtonClassName = 'slider-nav-button';
...
class Slider {
    constructor(element, options = {}) {
        ...
        this.changeActiveDot = this.changeActiveDotClass.bind(this);
    ...
    manageHtml() {
        ...
        this.containerNode.innerHTML = `
            ...
            <div class="${SliderNavClassName}">
                <button class="${SliderNavButtonClassName} ${SliderNavLeftClassName}">PREV</button>
                <button class="${SliderNavButtonClassName} ${SliderNavRightClassName}">NEXT</button>
            </div>
        ...
        this.dotsNode.innerHTML = Array.from(Array(this.size).keys()).map((key) => (
            `<button class="${SliderDotClassName} ${key == this.currentSlide ? SliderDotActiveClassName : ''}"></button>`
        )).join('');
    ...
    // установка размеров для линии слайдера и каждого отдельного слайда
    setParameters() {
        const coordsContainer = this.containerNode.getBoundingClientRect();
        this.width = coordsContainer.width;
        this.maxX = -(this.size - 1) * (this.width + this.setttings.margin);
        this.x = -this.currentSlide * (this.width + this.setttings.margin);

        this.lineNode.style.width = `${this.size * (this.width + this.setttings.margin)}px`;
        Array.from(this.slideNodes).forEach((slideNode) => {
            slideNode.style.width = `${this.width}px`;
            slideNode.style.marginRight = `${this.setttings.margin}px`;
        });    
    }
    ...
    removeEvents() {
        ...
        this.dotNodes.removeEventListener('click', this.clickDots);
    ...
    startDrag(evt) {
        ...
        this.containerNode.classList.add(SliderDragCursorClassName);
    ...
    stopDrag() {
        ...
        this.containerNode.classList.remove(SliderDragCursorClassName);
    ...
    dragging(evt) {
        ...
        this.x = Math.max(Math.min(this.startX + dragShift, easing), this.maxX + easing)

        this.setStylePosition(dragShift);

        // изменить активный слайд на предыдущий
        if (dragShift > 20 && dragShift > 0 && this.currentSlide > 0 && !this.currentSlideWasChanged && this.currentSlide > 0) {
            this.currentSlideWasChanged = true;
            this.currentSlide--;
        }

        // изменить активный слайд на следующий
